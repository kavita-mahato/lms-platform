...
const CourseDetails = () => {
  ...
  const { calculateRating, calculateChapterTime,
    calculateTotalCourseDuration, calculateNoOfLectures, currency, backendUrl, userData } = useContext(AppContext);
    ...
    <>
      <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
        ...
        <div className='max-w-xl z-10 text-gray-500'>
          ...
          {/* Course Info */}
          <p className='text-sm'>Course by <span className='text-blue-600 underline'>{courseData.educator?.name || courseData.educator}</span></p>
          <div className='pt-8 text-gray-800'>
            <h2 className='font-semibold md:text-xl text-lg text-gray-800'>Course Structure</h2>
            <div className='pt-5'>
                    ...
                    <ul className='list-disc md:pl-10 pl-4 py-2 pr-4 text-gray-600 border-t border-gray-300'>
                      {chapter.chapterContent.map((lecture, lecIndex) => (
                        <li key={lecIndex} className='flex items-start gap-2 py-1 cursor-pointer hover:text-gray-800'>
                          <img src={assets.play_icon} alt="play icon" className="inline mt-1 w-4 h-4" />
        ...
        <div className='max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-75 sm:min-w-105'>
          ...
          <div className='p-5'>
            <div className='flex item-center gap-2'>
              ...
              <p className='text-red-500'><span className='font-medium'>5 days</span> days left at this price</p>
            </div>
            ...
            <button onClick={enrollCourse} className='bg-blue-500 text-white font-medium mt-4 w-full md:mt-6 py-3 hover:bg-blue-600 transition-colors cursor-pointer'>
