...
const AddCourse = () => {
          ...
          const newLecture = {
            ...
            lectureOrder:
              ch.chapterContent.length > 0
                ? ch.chapterContent.slice(-1)[0].lectureOrder +
                1
                : 1,
          };
          return {
  ...
  useEffect(() => {
    if (!quillRef.current && editorRef.current) {
      quillRef.current = new Quill(editorRef.current, {
        theme: 'snow',
        placeholder:
          'Write a compelling description for your course...',
        modules: {
          toolbar: [
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="bg-white rounded-lg border border-gray-200 overflow-hidden focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-500 transition-all">
                <div
                  ref={editorRef}
                  className="min-h-62.5 border-none"
                />
              </div>
            </div>
          </div>
      ...
      case 1:
        return (
          <div className="animate-fadeIn">
            ...
            <label
              className={`flex flex-col items-center justify-center w-full h-80 border-2 border-dashed rounded-xl cursor-pointer hover:bg-gray-50 transition-all ${image ? 'border-blue-200 bg-blue-50/30' : 'border-gray-300'}`}
            >
              <input
                type="file"
                hidden
          ...
          <div className="space-y-6 animate-fadeIn">
            ...
            <div className="space-y-3">
              {chapters.map((ch, i) => (
                <div
                  key={ch.chapterId}
                  className="border border-gray-200 rounded-xl bg-white overflow-hidden shadow-sm transition-shadow hover:shadow-md"
                >
                  <div
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className={`transform transition-transform duration-300 text-gray-400 ${!ch.collapsed ? 'rotate-180' : ''}`}
                      >
                  ...
                  {!ch.collapsed && (
                    <div className="bg-gray-50/50 border-t border-gray-100 p-3 space-y-2">
                      {ch.chapterContent.map(
                        (lec, idx) => (
                          <div
                            key={lec.lectureId}
                            className="flex justify-between items-center bg-white p-3 rounded-lg border border-gray-100 shadow-sm pl-4 ml-4 relative"
                          >
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 text-sm font-medium text-blue-600 hover:text-blue-700 mt-2 ml-4 pl-2 py-1 transition-colors"
                      >
        ...
        return (
          <div className="bg-gray-50 rounded-xl p-6 border border-gray-200 animate-fadeIn space-y-4">
            ...
            <button
              ...
              className="w-full bg-black text-white h-12 rounded-lg font-medium text-lg shadow-lg hover:bg-gray-900 transform hover:-translate-y-0.5 transition-all mt-6"
            >
              Publish Course
            </button>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-start justify-center p-4 md:p-10 font-sans text-gray-800">
      <div className="w-full max-w-4xl bg-white rounded-2xl shadow-xl shadow-gray-200/50 p-6 md:p-10 border border-gray-100 flex flex-col gap-8">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Fill in the details to publish your new course.
          </p>
        </div>
        ...
        <div className="relative flex justify-between w-full mb-4">
          ...
          <div
            className="absolute top-1/2 left-0 h-0.5 bg-black -z-10 -translate-y-1/2 rounded transition-all duration-300"
            style={{
          ...
          {steps.map((label, i) => (
            ...
            >
              <div
                className={`w-8 h-8 flex items-center justify-center rounded-full text-sm font-bold border-2 transition-all duration-300 ${i <= step ? 'bg-black text-white border-black' : 'bg-white text-gray-300 border-gray-200'}`}
              >
                {i < step ? <Icons.Check /> : i + 1}
              </div>
              <span
                className={`text-xs font-medium uppercase tracking-wide hidden md:block transition-colors ${i <= step ? 'text-black' : 'text-gray-300'}`}
              >
                {label}
              </span>
            </div>
        ...
        <div className="flex justify-between pt-6 border-t border-gray-100">
          <button
            onClick={back}
            disabled={step === 0}
            className="px-6 py-2.5 rounded-lg border border-gray-200 text-gray-600 font-medium hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
          >
            Back
          </button>
          {step < steps.length - 1 && (
            <button
              onClick={next}
              className="px-8 py-2.5 rounded-lg bg-black text-white font-medium hover:bg-gray-800 shadow-md hover:shadow-lg transition-all flex items-center gap-2"
            >
              Next{' '}
              <Icons.ChevronDown className="rotate-[-90deg] w-4 h-4" />
            </button>
          )}
        </div>
      </div>
      ...
      {showChapterPopup && (
        <Modal
          onClose={() => setShowChapterPopup(false)}
          title="Add Chapter"
        >
      ...
      {showLecturePopup && (
        <Modal
          onClose={() => setShowLecturePopup(false)}
          title="Add Lecture"
        >
          <div className="space-y-4">
            <InputField
            ...
            <div className="flex items-center gap-2 pt-2">
              <input
                ...
                checked={lectureDetails.isPreviewAvailable}
                onChange={(e) =>
                  setLectureDetails({
                    ...lectureDetails,
                    isPreviewAvailable: e.target.checked,
                  })
                }
                className="w-4 h-4 rounded border-gray-300 text-black focus:ring-black"
              />
              <label
                htmlFor="preview-available"
                className="text-sm text-gray-600 cursor-pointer"
              >
                Make this lecture available for preview
              </label>
            </div>

            <button
              onClick={addLecture}
              className="w-full bg-black text-white py-2.5 rounded-lg font-medium hover:bg-gray-800 transition-colors mt-2"
            >
              Add Lecture
            </button>
          </div>
